...
class TestMCPServerIntegration:
    ...
    async def test_read_server_resource(self, server, mock_connection_manager):
        ...
        mock_connection_manager.get_status_all.return_value = {
            "test-server": mock_status
        }
    ...
    async def test_exec_command_tool(
        self, server, mock_connection_manager, mock_command_result
    ):
    ...
    async def test_read_file_tool(
        self, server, mock_connection_manager, sample_file_content
    ):
        ...
        result = await server.call_tool(
            "read_file",
            {"server_name": "test-server", "file_path": "/test/file.txt"},
        )
    ...
    async def test_list_directory_tool(self, server, mock_connection_manager):
        ...
        result = await server.call_tool(
            "list_directory",
            {"server_name": "test-server", "directory_path": "/test"},
        )
    ...
    async def test_system_info_tool(self, server, mock_connection_manager):
        ...
        result = await server.call_tool("system_info", {"server_name": "test-server"})

        assert len(result) == 1
        result_data = json.loads(result[0].text)
        assert (
            "cpu" in result_data or "memory" in result_data
    ...
    async def test_security_validation_integration(
        self, server, mock_connection_manager
    ):
        ...
        with patch("src.vps_manager.security.SecurityValidator") as MockValidator:
            ...
            with pytest.raises(Exception, match="Dangerous command detected"):
                await server.call_tool(
                    "exec_command",
                    {"server_name": "test-server", "command": "rm -rf /"},
                )
        ...
        with patch("src.vps_manager.security.SecurityValidator") as MockValidator:
            ...
            with pytest.raises(Exception, match="Path not allowed"):
                await server.call_tool(
                    "read_file",
                    {"server_name": "test-server", "file_path": "/etc/shadow"},
                )
    ...
    async def test_connection_error_handling(self, server, mock_connection_manager):
        ...
        with pytest.raises(Exception, match="Failed to get connection"):
            await server.call_tool(
                "exec_command", {"server_name": "test-server", "command": "ls"}
            )
    ...
    async def test_concurrent_tool_execution(self, server, mock_connection_manager):
        ...
        mock_conn1.connection.run = AsyncMock(
            return_value=MagicMock(returncode=0, stdout="result1")
        )

        mock_conn2 = MagicMock()
        mock_conn2.connection.run = AsyncMock(
            return_value=MagicMock(returncode=0, stdout="result2")
        )

        # Return different connections for concurrent requests
        mock_connection_manager.get_connection.side_effect = [
            mock_conn1,
            mock_conn2,
        ]

        # Execute two commands concurrently
        task1 = server.call_tool(
            "exec_command",
            {"server_name": "test-server", "command": "echo test1"},
        )

        task2 = server.call_tool(
            "exec_command",
            {"server_name": "test-server", "command": "echo test2"},
        )
