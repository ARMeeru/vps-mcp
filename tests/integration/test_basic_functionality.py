...
class TestServerInitialization:
    ...
    def test_server_handlers_registration(self, sample_vps_config):
        ...
        # Check that handlers are registered (we can't easily test the actual registration
...
class TestLoggingSetup:
    ...
    def test_logging_setup(self, temp_directory):
        ...
        for level in ["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"]:
            ...
            debug_log = Path(log_dir) / "debug.log"
            error_log = Path(log_dir) / "error.log"
            
            # Files may not exist until first log, but directory should be there
...
class TestSecurityIntegration:
    ...
    def test_command_validation_comprehensive(self, security_validator):
        ...
        test_cases = [
            ...
            ("ls -la /home/testuser", True, "Basic list command in allowed path"),
            ("cat /tmp/test/file.txt", True, "Read file in allowed path"),
            ("echo 'hello' > /home/testuser/output.txt", True, "Redirect to allowed path"),
            ("rm -rf /", False, "Dangerous deletion command"),
            ("sudo rm /etc/passwd", False, "Dangerous sudo command"),
            ("find /home/testuser -name '*.log' | head -10", True, "Safe piped command"),
            (":(){ :|:& };:", False, "Fork bomb"),
            ("test_blocked_command arg", False, "Custom blocked command"),
            ("systemctl restart nginx", True, "Valid service command"),
            ("chmod 777 /home/testuser/file.txt", True, "Safe permission change"),
        ...
        for command, expected_valid, description in test_cases:
            ...
            assert is_valid == expected_valid, f"Failed for: {description} - Command: {command}"
