...
class TestCommandTool:
    ...
    @pytest.mark.asyncio
    async def test_exec_command_success(self, mock_connection_manager, mock_command_result):
    ...
    @pytest.mark.asyncio
    async def test_exec_command_with_timeout(self, mock_connection_manager, mock_command_result):
        ...
        result = await tool.exec_command("test-server", "sleep 5", timeout=10)
    ...
    @pytest.mark.asyncio
    async def test_exec_command_failure(self, mock_connection_manager, mock_failed_command_result):
    ...
    async def test_exec_command_security_validation(self, mock_connection_manager):
        ...
        with patch('src.vps_manager.security.SecurityValidator') as MockValidator:
            ...
            validator_instance.validate_command.return_value = False, "Dangerous command"
...
class TestFileOperationsTool:
    ...
    @pytest.mark.asyncio
    async def test_read_file_success(self, mock_connection_manager, sample_file_content):
    ...
    @pytest.mark.asyncio
    async def test_read_binary_file(self, mock_connection_manager, sample_binary_content):
    ...
    @pytest.mark.asyncio
    async def test_write_file_success(self, mock_connection_manager, sample_file_content):
        ...
        result = await tool.write_file("test-server", "/test/output.txt", sample_file_content)
        
        assert result["success"] is True
        assert result["bytes_written"] == len(sample_file_content.encode())
        
        mock_sftp.writefile.assert_called_once_with("/test/output.txt", sample_file_content.encode())

    @pytest.mark.asyncio
    async def test_write_file_with_permissions(self, mock_connection_manager, sample_file_content):
        ...
        result = await tool.write_file("test-server", "/test/script.sh", sample_file_content, permissions=0o755)
    ...
    async def test_security_validation_write(self, mock_connection_manager):
        ...
        with patch('src.vps_manager.security.SecurityValidator') as MockValidator:
            ...
            with pytest.raises(Exception, match="Path not allowed"):
                await tool.write_file("test-server", "/root/.ssh/authorized_keys", "malicious content")
...
class TestMonitoringTool:
    ...
    @pytest.mark.asyncio
    async def test_get_system_info_success(self, mock_connection_manager, system_status_data):
        ...
        mem_result.stdout = "MemTotal:    8388608 kB\nMemAvailable: 4194304 kB\nMemFree:     2097152 kB"
        
        disk_result = MagicMock()
        disk_result.returncode = 0
        disk_result.stdout = "Filesystem     1K-blocks      Used Available Use% Mounted on\n/dev/sda1      104857600  52428800  52428800  50% /"
...
class TestServiceManagementTool:
    ...
    async def test_list_services_systemd(self, mock_connection_manager):
        ...
        systemctl_result.stdout = "nginx.service loaded active running nginx web server\nssh.service loaded active running OpenBSD Secure Shell server"
    ...
    @pytest.mark.asyncio
    async def test_security_validation_service_management(self, mock_connection_manager):
