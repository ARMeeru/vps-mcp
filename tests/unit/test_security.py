...
class TestSecurityValidator:
    ...
    def test_validate_command_safe_commands(self):
        ...
        for command in safe_commands:
            ...
            assert is_valid, f"Command '{command}' should be valid but got error: {error}"
    ...
    def test_validate_file_path_dangerous_extensions(self):
        ...
        for path in dangerous_files:
            is_valid, error, resolved = self.validator.validate_file_path(path, "execute")
            assert not is_valid, f"Dangerous file '{path}' should not be executable"
    
    def test_validate_file_size(self):
        """Test file size validation."""
        # Test valid size
        is_valid, error = self.validator.validate_file_size(1024 * 1024, 2)  # 1MB file, 2MB limit
        assert is_valid
        
        # Test oversized file
        is_valid, error = self.validator.validate_file_size(5 * 1024 * 1024, 2)  # 5MB file, 2MB limit
        assert not is_valid
