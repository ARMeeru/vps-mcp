...
from asyncssh import ConnectionLost, PermissionDenied

from src.vps_manager.connection_pool import ConnectionManager, ConnectionPool, SSHConnection, ConnectionState
...
class TestConnectionPool:
    ...
    @pytest.mark.asyncio
    async def test_get_connection_reuses_healthy(self, sample_server_config, mock_ssh_connection):
    ...
    @pytest.mark.asyncio
    async def test_get_connection_removes_unhealthy(self, sample_server_config, mock_ssh_connection):
        ...
        with patch.object(unhealthy_conn, 'is_healthy', return_value=False), \
             patch.object(unhealthy_conn, 'close', new_callable=AsyncMock), \
             patch('asyncssh.connect', new_callable=AsyncMock, return_value=mock_new_connection):
    ...
    @pytest.mark.asyncio
    async def test_release_connection_closes_excess(self, sample_server_config, mock_ssh_connection):
    ...
    @pytest.mark.asyncio
    async def test_cleanup_all_connections(self, sample_server_config, mock_ssh_connection):
        """Test cleanup closes all connections."""
        pool = ConnectionPool(sample_server_config, max_connections=3)
        
        # Add some connections
        conn1 = SSHConnection(connection=MagicMock(), server_name="test", connection_id="1")
        conn2 = SSHConnection(connection=MagicMock(), server_name="test", connection_id="2")
    ...
    def test_get_status(self, sample_server_config, mock_ssh_connection):
        ...
        with patch.object(healthy_conn, 'is_healthy', return_value=True), \
...
class TestConnectionManager:
    ...
    async def test_init(self, sample_vps_config):
        ...
        assert manager.pools["test-server"].server_config == sample_vps_config.servers[0]
    ...
    async def test_get_connection_success(self, sample_vps_config, mock_ssh_connection):
        ...
        with patch.object(manager.pools["test-server"], 'get_connection', new_callable=AsyncMock) as mock_get:
    ...
    async def test_release_connection(self, sample_vps_config, mock_ssh_connection):
        ...
        with patch.object(manager.pools["test-server"], 'release_connection', new_callable=AsyncMock) as mock_release:
    ...
    def test_get_status_all(self, sample_vps_config):
        ...
        with patch.object(manager.pools["test-server"], 'get_status', return_value=mock_status):
    ...
    async def test_cleanup_all_pools(self, sample_vps_config):
        ...
        with patch.object(manager.pools["test-server"], 'cleanup', new_callable=AsyncMock) as mock_cleanup:
    ...
    async def test_health_check_removes_unhealthy_connections(self, sample_vps_config):
        ...
        with patch.object(manager.pools["test-server"], 'cleanup_unhealthy_connections', new_callable=AsyncMock) as mock_cleanup:
