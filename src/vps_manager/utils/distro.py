...
class DistroDetector:
    ...
    DISTRO_PATTERNS = {
        DistroFamily.DEBIAN: [
            r"ubuntu", r"debian", r"mint", r"elementary", r"pop",
            r"kali", r"parrot", r"mxlinux"
        ],
        DistroFamily.REDHAT: [
            r"rhel", r"red hat", r"centos", r"fedora", r"amazon linux",
            r"oracle linux", r"rocky", r"alma"
        ],
        DistroFamily.ARCH: [
            r"arch", r"manjaro", r"antergos", r"endeavour"
        ],
        DistroFamily.SUSE: [
            r"opensuse", r"suse", r"sles"
        ]
    }
    
    @staticmethod
    async def detect_init_system(connection) -> InitSystem:
        """Detect the init system running on the server.
        ...
        try:
            ...
            result = await connection.run("ps -p 1 -o comm= 2>/dev/null || ps -p 1 -o args= | head -1", check=True)
            ...
            if "systemd" in pid1_command:
                ...
                # Check if systemctl is available (might be systemd)
                try:
                    await connection.run("which systemctl", check=True)
                    return InitSystem.SYSTEMD
                except:
                    pass
                ...
                try:
                    await connection.run("which initctl", check=True)
                    return InitSystem.UPSTART
                except:
                    pass
        ...
        try:
            # Check for systemctl
            await connection.run("which systemctl && systemctl --version", check=True)
            return InitSystem.SYSTEMD
        except:
            pass
        ...
        try:
            # Check for upstart
            await connection.run("which initctl && initctl version", check=True)
            return InitSystem.UPSTART
        except:
            pass
        ...
        try:
            # Check for SysV init
            await connection.run("ls /etc/init.d/", check=True)
            return InitSystem.SYSVINIT
        except:
            pass
    ...
    @staticmethod
    async def detect_distro_family(connection) -> Tuple[DistroFamily, str]:
    ...
    @staticmethod
    async def _detect_from_os_release(connection) -> Tuple[DistroFamily, str]:
        ...
        for family, patterns in DistroDetector.DISTRO_PATTERNS.items():
            for pattern in patterns:
                if re.search(pattern, distro_id) or re.search(pattern, distro_name.lower()):
                    return family, distro_name
        
        return DistroFamily.UNKNOWN, distro_name
    
    @staticmethod
    async def _detect_from_lsb_release(connection) -> Tuple[DistroFamily, str]:
    ...
    @staticmethod
    async def _detect_from_etc_files(connection) -> Tuple[DistroFamily, str]:
    ...
    @staticmethod
    async def _detect_from_uname(connection) -> Tuple[DistroFamily, str]:
...
class ServiceCommandMapper:
    ...
    @staticmethod
    def get_command(init_system: InitSystem, action: str, service: str, 
                   distro_family: DistroFamily = DistroFamily.UNKNOWN) -> Optional[str]:
        ...
        if (init_system == InitSystem.SYSVINIT and 
            distro_family == DistroFamily.REDHAT and 
            action in ServiceCommandMapper.REDHAT_SYSV_COMMANDS):
            command_template = ServiceCommandMapper.REDHAT_SYSV_COMMANDS[action]
        else:
            command_template = commands.get(action)
    ...
    def normalize_service_name(service: str, init_system: InitSystem) -> str:
        ...
        if init_system == InitSystem.SYSTEMD:
            # For systemd, ensure .service suffix if not present and not a target/socket
            if not any(service.endswith(ext) for ext in ['.service', '.target', '.socket', '.timer', '.mount']):
