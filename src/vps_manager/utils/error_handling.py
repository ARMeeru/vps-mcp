...
class ErrorHandler:
    ...
    def handle_connection_error(
        ...
        error_mapping = {
            "PermissionDenied": {
                ...
                "user_action": "Check your SSH key permissions and server configuration. Ensure the public key is added to the server's authorized_keys file."
            },
            "ConnectionRefused": {
                "category": ErrorCategory.CONNECTION,
                "severity": ErrorSeverity.HIGH,
                "message": f"Connection refused by server '{server_name}'",
                "user_action": "Verify the server is running, the hostname/IP is correct, and SSH port is open. Check firewall settings."
            },
            "NoValidConnectionsError": {
                "category": ErrorCategory.CONNECTION,
                "severity": ErrorSeverity.HIGH,
                "message": f"No valid connections available for server '{server_name}'",
                "user_action": "Check network connectivity and server availability. Verify SSH service is running on the server."
            },
            "ConnectionLost": {
                "category": ErrorCategory.CONNECTION,
                "severity": ErrorSeverity.MEDIUM,
                "message": f"Connection lost to server '{server_name}'",
                "user_action": "The connection was dropped. This may be temporary - try again. Check network stability."
            },
            "TimeoutError": {
                "category": ErrorCategory.TIMEOUT,
                "severity": ErrorSeverity.MEDIUM,
                "message": f"Connection timeout to server '{server_name}'",
                "user_action": "The server is not responding. Check network connectivity and server status."
            }
        }
        
        exception_type = type(exception).__name__
        error_info = error_mapping.get(exception_type, {
            "category": ErrorCategory.CONNECTION,
            "severity": ErrorSeverity.MEDIUM,
            "message": f"Connection error for server '{server_name}': {str(exception)}",
            "user_action": "Check your network connection and server configuration."
        })
    ...
    def handle_command_error(
        ...
        if exit_code is not None and exit_code != 0:
            return VPSManagerError(
                ...
                message=f"Command failed on server '{server_name}' with exit code {exit_code}",
                technical_details=f"Command: {command}\nExit code: {exit_code}\nError: {str(exception)}",
                user_action=f"The command '{command}' failed. Check the command syntax and server state.",
                server_name=server_name,
        ...
        if "Permission denied" in str(exception):
            return VPSManagerError(
                ...
                severity=ErrorSeverity.MEDIUM,
                message=f"Permission denied executing command on server '{server_name}'",
                technical_details=f"Command: {command}\nError: {str(exception)}",
                user_action="Check user permissions. You may need sudo privileges or different user account for this operation.",
                server_name=server_name,
                operation="exec_command",
        ...
        return VPSManagerError(
            ...
            technical_details=f"Command: {command}\nError: {str(exception)}",
            user_action="Check the command syntax and ensure the server is properly configured.",
            server_name=server_name,
            operation="exec_command",
    ...
    def handle_file_operation_error(
        ...
        if "Permission denied" in str(exception):
            return VPSManagerError(
                ...
                technical_details=f"Operation: {operation}\nPath: {file_path}\nError: {str(exception)}",
                user_action=f"Check file permissions for '{file_path}'. You may need different user privileges.",
                server_name=server_name,
                operation=operation,
        ...
        if "No space left" in str(exception):
            return VPSManagerError(
                ...
                technical_details=f"Operation: {operation}\nPath: {file_path}\nError: {str(exception)}",
                user_action="Free up disk space on the server or use a different location.",
                server_name=server_name,
                operation=operation,
    ...
    def handle_security_error(
        ...
        return VPSManagerError(
            ...
            user_action="This operation was blocked for security reasons. Review your command or file path.",
            server_name=server_name,
            operation=operation,
    ...
    def handle_service_error(
        ...
        if "Unit not found" in str(exception):
            return VPSManagerError(
                ...
                technical_details=f"Action: {action}\nService: {service_name}\nError: {str(exception)}",
                user_action=f"Verify the service name '{service_name}' is correct and installed on the server.",
                server_name=server_name,
                operation="service_management",
                error_code="SERVICE_NOT_FOUND"
            )
        
        if "Failed to" in str(exception) and action in ["start", "stop", "restart"]:
            return VPSManagerError(
                category=ErrorCategory.SERVICE,
                severity=ErrorSeverity.MEDIUM,
                message=f"Failed to {action} service '{service_name}' on server '{server_name}'",
                technical_details=f"Action: {action}\nService: {service_name}\nError: {str(exception)}",
                user_action=f"Check service configuration and dependencies for '{service_name}'. Review service logs for details.",
                server_name=server_name,
                operation="service_management",
        ...
        return VPSManagerError(
            category=ErrorCategory.SERVICE,
            severity=ErrorSeverity.MEDIUM,
            message=f"Service management error for '{service_name}' on server '{server_name}'",
            technical_details=f"Action: {action}\nService: {service_name}\nError: {str(exception)}",
            ...
            try:
                ...
                if "connection" in operation.lower():
                    ...
                    error = ErrorHandler.handle_file_operation_error(
                        server_name or "unknown",
                        operation,
                        kwargs.get('file_path') or kwargs.get('directory_path', 'unknown'),
                        e
                    )
...
def validate_server_name(server_name: str, available_servers: list) -> Optional[VPSManagerError]:
...
def validate_required_params(params: Dict[str, Any], required: list) -> Optional[VPSManagerError]:
