...
class SecurityValidator:
    ...
    def __init__(self, allowed_paths: List[str], additional_blocked_commands: List[str] = None):
        ...
        self.compiled_sudo_patterns = [re.compile(pattern, re.IGNORECASE)
                                     for pattern in self.DANGEROUS_SUDO_PATTERNS]
        
        # Add additional blocked commands
        if additional_blocked_commands:
            additional_patterns = [re.compile(pattern, re.IGNORECASE)
                                 for pattern in additional_blocked_commands]
            self.compiled_patterns.extend(additional_patterns)
        
        logger.info(f"Security validator initialized with {len(self.allowed_paths)} allowed paths")
    
    def validate_command(self, command: str, max_length: int = 10000) -> Tuple[bool, Optional[str]]:
        ...
        if 'sudo' in normalized_command.lower():
            for pattern in self.compiled_sudo_patterns:
                if pattern.search(normalized_command):
                    return False, f"Sudo command matches dangerous pattern: {pattern.pattern}"
        ...
        for chain in dangerous_chains:
            if chain in command:
                ...
                if not self._is_safe_chaining(command, chain):
                    logger.warning(f"Command contains potentially dangerous chaining: {chain}")
    ...
    def validate_file_path(self, file_path: str, operation: str = "access") -> Tuple[bool, Optional[str], Path]:
        ...
        try:
            ...
            if not is_allowed:
                return False, f"Path not in allowed directories: {resolved_path}", resolved_path
            
            # Check file extension for execution operations
            if operation == "execute" and resolved_path.suffix.lower() in self.DANGEROUS_EXTENSIONS:
                return False, f"Dangerous file extension for execution: {resolved_path.suffix}", resolved_path
            
            # Additional checks for special files
            if self._is_special_file(resolved_path):
                if operation in ["write", "execute"]:
                    return False, f"Cannot {operation} special file: {resolved_path}", resolved_path
    ...
    def _is_special_file(self, path: Path) -> bool:
        ...
        """
        special_paths = [
            '/dev', '/proc', '/sys', '/boot', '/etc/passwd', '/etc/shadow',
            '/etc/sudoers', '/etc/ssh', '/root'
        ]
    ...
    def validate_file_size(self, file_size: int, max_size_mb: int) -> Tuple[bool, Optional[str]]:
        ...
        if file_size > max_size_bytes:
            return False, f"File size {file_size} exceeds limit of {max_size_mb}MB"
    ...
    def check_sudo_requirements(self, command: str) -> bool:
        ...
        sudo_required_commands = [
            ...
            'ufw', 'apt', 'yum', 'dnf', 'pacman', 'nginx', 'apache2'
        ]
        
        # Paths that typically require sudo
        sudo_required_paths = [
            '/etc/', '/var/log/', '/usr/', '/opt/', '/boot/', '/sys/'
        ]
    ...
    def create_safe_command(self, base_command: str, user_args: List[str], 
                          use_sudo: bool = False, sudo_password: Optional[str] = None) -> str:
...
def validate_command_safe(command: str, allowed_paths: List[str], 
...
def validate_path_safe(file_path: str, allowed_paths: List[str], 
