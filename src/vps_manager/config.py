...
class ServerConfig(BaseModel):
    ...
    ssh_key_passphrase_env: Optional[str] = Field(
        default=None, description="Environment variable name for SSH key passphrase"
    )
...
def load_config(config_path: Optional[str] = None) -> VPSManagerConfig:
    ...
    for key, value in env_overrides.items():
        if value is not None:
            ...
            if key in ["max_connections_per_server", "health_check_interval", 
...
def get_ssh_key_passphrase(env_var_name: Optional[str]) -> Optional[str]:
    ...
    if passphrase is None:
        raise ValueError(f"SSH key passphrase environment variable not found: {env_var_name}")
