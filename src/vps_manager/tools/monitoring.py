...
from ..utils.mcp_responses import MCPMonitoringError, MCPResponse
...
class SystemMonitoringTool:
    ...
    async def get_system_status(
        self, server: Optional[str] = None, detailed: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                load_average = [
                    load_avg_data.get("1min", 0.0),
                    load_avg_data.get("5min", 0.0),
                    load_avg_data.get("15min", 0.0),
                ]
        ...
        try:
            ...
            try:
                result = await conn.connection.run(
                    "top -bn1 | grep 'Cpu(s)' | head -1", check=True
                )
    ...
    async def _get_process_info(self, conn: SSHConnection) -> Dict[str, Any]:
        ...
        try:
            result = await conn.connection.run(
                "ps aux --no-headers | wc -l", check=True
            )
    ...
    def clear_cache(self, server: Optional[str] = None) -> None:
        ...
        if server:
            keys_to_remove = [
                k for k in self._cache.keys() if k.startswith(f"{server}-")
            ]
