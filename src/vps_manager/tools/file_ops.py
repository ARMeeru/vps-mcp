...
class FileOperationsTool:
    ...
    async def read_file(
        ...
        try:
            ...
            if not conn:
                raise FileOperationError(f"No available connections for server {server}")
            ...
            try:
                ...
                try:
                    ...
                    # Validate file size
                    is_size_valid, size_error = validator.validate_file_size(file_size, max_size_mb)
    ...
    async def upload_file(
        ...
        try:
            ...
            # Security validation
            validator = SecurityValidator(server_config.allowed_paths)
            is_valid, error, resolved_path = validator.validate_file_path(remote_path, "write")
            if not is_valid:
                raise PathSecurityError(error)
            ...
            try:
                ...
                try:
                    ...
                    if remote_stat.size != file_size:
                        raise FileOperationError(f"Upload verification failed: size mismatch")
    ...
    async def download_file(
        ...
        try:
            ...
            # Security validation
            validator = SecurityValidator(server_config.allowed_paths)
            is_valid, error, resolved_path = validator.validate_file_path(remote_path, "read")
            if not is_valid:
                raise PathSecurityError(error)
            ...
            try:
                ...
                try:
                    ...
                    if local_stat.st_size != file_size:
                        raise FileOperationError(f"Download verification failed: size mismatch")
    ...
    async def _ensure_parent_dirs(self, sftp: asyncssh.SFTPClient, file_path: Path) -> None:
