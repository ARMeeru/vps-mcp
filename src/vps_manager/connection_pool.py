...
class SSHConnection:
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            ...
            and not getattr(self.connection, "_closing", False)
...
class ConnectionPool:
    ...
    def __init__(self, server_config: ServerConfig, pool_size: int = 3):
        ...
        logger.info(
            f"Initialized connection pool for {
                server_config.name} with {pool_size} connections"
        )
    ...
    async def get_connection(self, priority: str = "NORMAL") -> Optional[SSHConnection]:
        ...
        async with self._lock:
            ...
            if not ready_connections:
                logger.warning(
                    f"No ready connections available for {
        ...
        async with self._lock:
            if conn in self.connections:
                if conn.is_healthy():
                    ...
                    logger.warning(
                        f"Released unhealthy connection {conn.connection_id}"
                    )
    ...
    async def _connect_single(self, conn: SSHConnection) -> bool:
        ...
        if self._shutdown:
            logger.debug(
                f"Skipping connection {
                    conn.connection_id} - pool is shutting down"
            )
            return False
        ...
        try:
            ...
            result = await conn.connection.run(
                "echo 'connection-test'", check=True, timeout=10
            )
            ...
            logger.info(
                f"Successfully connected {
                    conn.connection_id} (attempt after {
    ...
    async def _connect_all(self) -> None:
        ...
        if tasks:
            ...
            logger.info(
                f"Connected {successful}/{len(tasks)} connections for "
            ...
            for conn in self.connections:
                ...
                if conn.connection and not getattr(conn.connection, "_closing", False):
                    try:
                        ...
                        if conn.connection and not getattr(
                            conn.connection, "_closing", False
                        ):
        ...
        if conn.connection:
            if not getattr(conn.connection, "_closing", False):
                try:
                    ...
                    logger.debug(
                        f"Error closing old connection {
                            conn.connection_id}: {e}"
                    )
            conn.connection = None
    ...
    async def _cleanup_failed_connection(self, conn: SSHConnection) -> None:
        ...
        if conn.connection:
            try:
                ...
                logger.warning(
                    f"Timeout cleaning up failed connection {
                        conn.connection_id}"
                )
            except Exception as e:
                logger.debug(
                    f"Error during connection cleanup {
                        conn.connection_id}: {e}"
                )
            finally:
                conn.connection = None
...
class ConnectionManager:
    ...
    async def get_connection(
        self, server_name: str, priority: str = "NORMAL"
    ...
    def get_status_all(self) -> Dict[str, Dict[str, any]]:
        ...
        """
        return {
            server_name: pool.get_status() for server_name, pool in self.pools.items()
        }
